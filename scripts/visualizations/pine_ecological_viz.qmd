...
# UCBN
pine <- fiveneedlepine::loadPine("M:/STAFF/FosterI/UCBNPineData/UCBN_LimberPine_BackEnd_Intermediate_QAQC_2023_Copy.accdb")

# KLMN
#pine <- fiveneedlepine::loadPine("C:\\Users\\ifoster\\Documents\\R\\mojn-pine-rpackage\\data\\KLMN_PineData\\KLMN_WBP_Master_2012_2023_20231221_PII_Copy.accdb")
...
# pine$data$LiveTrees <- pine$data$LiveTrees %>% filter(network == 'UCBN')
# pine$data$DeadTrees <- pine$data$DeadTrees %>% filter(network == 'UCBN')
```
...
densityPlotGraph <- pine$data$DeadTrees %>%
  ...
  labs(title = "Density by Plot",
       ...
       fill = "Visit") + 
  theme(plot.title = element_text(hjust = 0.5),
        axis.text.x = element_text(hjust = 0.5, angle = 315),
         strip.text = element_text(size = 6)) 
# +
#   #theme(axis.text.x = element_text(hjust = 0.5, angle = 315)) +
#   scale_fill_light()
...
crownKill <- cankersData %>%
  ...
  summarise(avgSpeciesCrownKill = mean(avgCrownKill),
            n = n()) %>%
  filter(n > 4) %>%
  ggplot(aes(x = scientificName, y = avgSpeciesCrownKill, fill = factor(visitNumber))) +
  geom_col(position = position_dodge(preserve = "single")) + 
  theme_minimal() + 
...
inactiveCankerPercent <- cankersData %>%
  ...
  ggplot(aes(x = factor(visitNumber), y = avgPercent, fill = inactiveCankers)) +
  geom_col() +
  facet_wrap(~scientificName) + 
  theme_minimal() + 
...
cankerPercent <- cankersData %>%
  ...
  summarise(avgPercent = mean(percent)) %>%
  filter(anyCankers == 'Y') %>%
  ggplot(aes(x = factor(visitNumber), y = avgPercent, fill = anyCankers)) +
  geom_col() +
  facet_wrap(~scientificName) + 
  theme_minimal() + 
...
activeCankerDensity <-  cankersData %>%
  mutate(DBHGroup = cut(x = treeDBH_cm, breaks = 5*(0:(max(treeDBH_cm)/5)))) %>%
  group_by(visitNumber, scientificName, anyCankers, locationID, DBHGroup) %>%
  summarise(plotDensity = n()/0.25) %>%
  group_by(visitNumber, scientificName, anyCankers, DBHGroup) %>%
  summarise(avgDensity = mean(plotDensity)) %>%
  filter(anyCankers == 'Y') %>%
  group_by(scientificName) %>%
  mutate(n = n()) %>%
  ...
  geom_line() +
  #geom_col() +
  #facet_wrap(~scientificName, scales = "free", strip.position = "bottom") + 
  theme_minimal() + 
  labs(title = "Canker Density",
       x = "DBH Group", y = "Density (#/ha)",
       color = "Visit") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(axis.text.x = element_text(hjust = 0.5, angle = 315)) +
  # scale_fill_light() +
...
# Data Wrangling to fix weird legend
#gp <- plotly::ggplotly(activeCankerDensity)
