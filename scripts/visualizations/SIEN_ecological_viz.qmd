...
countDeadGraph <- pine$data$DeadTrees %>%
  ...
  ggplot(aes(x = factor(visitNumber), y = count, fill = factor(scientificName))) +
  geom_col()  +
  facet_wrap(~sampleFrame, ncol = 10) + 
  theme_minimal() + 
...
densityPlotGraph <- pine$data$DeadTrees %>%
  group_by(sampleFrame, visitNumber) %>%
  mutate(numberOfPlots = n_distinct(locationID)) %>%
  group_by(scientificName, sampleFrame, visitNumber) %>%
  summarise(density = dplyr::n()/(first(numberOfPlots)*0.25)) %>%
...
mortalityCause <- pine$data$DeadTrees %>%
  filter(vitality == "Recently Dead") %>%
  group_by(sampleFrame, visitNumber, causeOfDeath) %>%
  summarise(n = dplyr::n()) %>%
  ggplot(aes(x = factor(visitNumber), y = n, fill = factor(causeOfDeath))) +
  geom_col() +
  facet_wrap(~sampleFrame, scales = "free_y") + 
  theme_minimal() + 
...
seedlingRelDensity <- pine$data$Seedling %>%
  getRelativeDensity(plotArea = 81, grouping = sampleFrame) %>% 
  filter(scientificName != "No seedlings" & scientificName != "Populus tremuloides") %>%
...
seedlingRelFrequency <- pine$data$Seedling %>%
  getFrequency(transectArea = 9, plotArea = 81, grouping = sampleFrame) %>% 
  filter(scientificName != "No seedlings" & scientificName != "Populus tremuloides") %>%
