...
# Extract the path of the first Access database in the KLMN folder inside the data folder folder of the project directory
# Or the database path can be input manually
database_dir <- common::file.find(here::here("data", "KLMN"), "*.accdb", up = 0)[1]
...
# targetSpecies <- c("Pinus albicaulis", "Pinus balfouriana", "Pinus flexilis", "Pinus longaeva")
# 
# summaryTable <- pine$data$Tree %>%
...
#     # Otherwise set it to N
#     .default = 'N')) %>%
#   group_by(visitNumber, locationID) %>%
#   # Calculate percent of cankers and pine beetles for each plot
#   summarise(numCanker = sum(anyCankers == 'Y'),
#          numNoCanker = sum(anyCankers == 'N'),
#          numBeetle = sum(pineBeetle == 'Y'),
#          numNoBeetle = sum(pineBeetle == 'N'),
#          Canker = numCanker/n()*100,
#          Beetle = numBeetle/n()*100) 
# 
# # Calculate the mean and median of dead target species trees for each visit
# # Use a new copy of pine so entries with NAs in canker fields are counted
# deadTreeSummary <- pine$data$Tree %>%
#   group_by(visitNumber, locationID) %>%
#   summarise(
#          numDeadTrees = sum((vitality == "Dead" | vitality == "Recently Dead") & (scientificName %in% targetSpecies) & (!is.na(vitality))), 
#          totalTrees = sum(scientificName %in% targetSpecies),
#          # percent of dead target species trees = (number of dead target species trees)/(number of target species trees)
#          Dead = numDeadTrees/totalTrees*100)
# 
# summaryTable <- summaryTable %>%
#   # Combine canker/beetle and dead tree table
#   full_join(deadTreeSummary, by = join_by(visitNumber, locationID)) %>%
#   pivot_longer(cols = c(Canker, Beetle, Dead), names_to = "category", values_to = "Percent") %>%
#   group_by(visitNumber, category) %>%
#   # Calculate mean and meadian for each visit and grouping
#   summarise(Mean = round(mean(Percent, na.rm = TRUE), 2),
#             Median = round(median(Percent, na.rm = TRUE), 2)) %>% 
#   # Add long description for each category
#   mutate(Description = case_when(
#     category == "Canker" ~ "Percent of trees with blister rust symptoms",
#     category == "Beetle" ~ "Percent of trees with pine beetle symptoms",
#     category == "Dead" ~ "Percent of dead target species trees")) %>%
#   pivot_longer(cols = c(Mean, Median), names_to = "Calculation", values_to = "Value") %>%
#   mutate(colNames = paste0("Visit ", visitNumber, " ", Calculation)) %>%
#   ungroup() %>%
#   select(-c(visitNumber, Calculation, category)) %>%
...
```{r}
# Split data up into CRLA and LAVO

# Filter for only CRLA/LAVO data
crla <- pine
crla$data$Seedling <- crla$data$Seedling %>% filter(park == 'CRLA')
crla$data$Site <- crla$data$Site %>% filter(park == 'CRLA')
crla$data$Tree <- crla$data$Tree %>% filter(park == 'CRLA')
crla$data$Visit <- crla$data$Visit %>% filter(park == 'CRLA')
crla$data$LiveTrees <- crla$data$LiveTrees %>% filter(park == 'CRLA')
crla$data$DeadTrees <- crla$data$DeadTrees %>% filter(park == 'CRLA')


lavo <- pine
lavo$data$Seedling <- lavo$data$Seedling %>% filter(park == 'LAVO')
lavo$data$Site <- lavo$data$Site %>% filter(park == 'LAVO')
lavo$data$Tree <- lavo$data$Tree %>% filter(park == 'LAVO')
lavo$data$Visit <- lavo$data$Visit %>% filter(park == 'LAVO')
lavo$data$LiveTrees <- lavo$data$LiveTrees %>% filter(park == 'LAVO')
lavo$data$DeadTrees <- lavo$data$DeadTrees %>% filter(park == 'LAVO')
```
...
# Calculate the mean and median of dead target species trees for each visit
# Use a new copy of pine so entries with NAs in canker fields are counted
deadTreeSummary <- crla$data$Tree %>%
  group_by(visitNumber, locationID) %>%
  summarise(
         numDeadTrees = sum((vitality == "Dead" | vitality == "Recently Dead") & (scientificName %in% targetSpecies) & (!is.na(vitality))), 
         totalTrees = sum(scientificName %in% targetSpecies),
...
densityGraph <- crla$data$LiveTrees %>%
  getDensityDBH() %>%
...
frequencyData <- crla$data$LiveTrees %>%
  getFrequency() %>%
...
# TODO: add error bars
#treeDominance <- getDominance(crla$data$LiveTrees)
...
```{r}
countGraph <- crla$data$LiveTrees %>%
...
```{r}
relativeDensityGraph <- crla$data$LiveTrees %>%
...
```{r}
# Original version
relativeFrequency <- crla$data$LiveTrees %>%
...
```{r}
relDomiance <- crla$data$LiveTrees %>%
...
```{r}
importanceVal <- crla$data$LiveTrees %>%
...
```{r}
countDeadGraph <- crla$data$DeadTrees %>%
  getTreeCount(grouping = sampleFrame) %>%
  #filter(grepl("CRLA", locationID)) %>%
  #filter(locationID == "CRLA_Alpine_CRWB01") %>%
  group_by(scientificName, visitNumber, sampleFrame) %>%
...
densityGraph <- crla$data$DeadTrees %>%
...
```{r}
densityPlotGraph <- crla$data$DeadTrees %>%
  group_by(visitNumber, sampleFrame) %>%
  mutate(numberOfPlots = n_distinct(locationID)) %>%
  #mutate(DBHGroup = cut(x = treeDBH_cm, breaks = 5*(0:(max(treeDBH_cm)/5)))) %>%
  group_by(scientificName, visitNumber, sampleFrame) %>%
  summarise(density = n()/(dplyr::first(numberOfPlots)*0.25)) %>%
  #summarise(density = n()/0.25) %>%
...
```{r}
countRecentlyDeadGraph <- crla$data$DeadTrees %>%
...
```{r}
densityRecentlyDead <- crla$data$DeadTrees %>%
  filter(vitality == "Recently Dead") %>%
  group_by(scientificName, locationID, visitNumber) %>%
  summarise(density = n()/0.25) %>%
  ggplot(aes(x = factor(visitNumber), y = density, fill = factor(scientificName))) +
...
treeVitality <- crla$data$Tree %>%
  ...
  summarise(alive = mean(vitality == "Live")*100,
            n = n()) %>%
  filter(n > 4 & scientificName != "Unknown" & scientificName != "Dead Tree") %>%
  mutate(DBHGroup = str_extract(DBHGroup, regex("^[^]]*"))) %>%
...
mortalityCause <- crla$data$DeadTrees %>%
  filter(vitality == "Recently Dead") %>%
...
mistletoePercentData <- crla$data$Tree %>%
...
mistletoeDensity <- crla$data$Tree %>%
  group_by(visitNumber) %>%
...
```{r}
femaleConePercent <- crla$data$Tree %>%
...
femaleConeDensity <- crla$data$Tree %>%
  filter(!is.na(femaleCones)) %>%
...
seedlingCount <- crla$data$Seedling %>%
...
seedlingDensity <- crla$data$Seedling %>%
  group_by(visitNumber) %>%
  mutate(totalPlots = n_distinct(locationID)) %>%
...
seedlingFrequency <- crla$data$Seedling %>%
...
seedlingRelDensity <- crla$data$Seedling %>%
...
```{r}
seedlingRelFrequency <- crla$data$Seedling %>%
  getFrequency(transectArea = 9, plotArea = 81) %>% 
  filter(scientificName != "No seedlings" & scientificName != "Populus tremuloides") %>%
...
# Calculate the mean and median of dead target species trees for each visit
# Use a new copy of pine so entries with NAs in canker fields are counted
deadTreeSummary <- lavo$data$Tree %>%
  group_by(visitNumber, locationID) %>%
  summarise(
         numDeadTrees = sum((vitality == "Dead" | vitality == "Recently Dead") & (scientificName %in% targetSpecies) & (!is.na(vitality))), 
         totalTrees = sum(scientificName %in% targetSpecies),
...
densityGraph <- lavo$data$LiveTrees %>%
  getDensityDBH() %>%
  group_by(scientificName) %>%
  mutate(n = n()) %>%
  # Filter out species that don't have enough data points to graph
...
```{r}
# Average frequency of each species - includes plots without a species when averaging
frequencyData <- lavo$data$LiveTrees %>%
  getFrequency() %>%
  group_by(visitNumber) %>%
  mutate(numberOfPlots = dplyr::n_distinct(locationID)) %>%
  group_by(scientificName, visitNumber) %>%
...
# avg basal area of plots split by communities/year
# TODO: add error bars
#treeDominance <- getDominance(lavo$data$LiveTrees)
...
```{r}
countGraph <- lavo$data$LiveTrees %>%
  getTreeCount(grouping = sampleFrame) %>%
  #filter(locationID == "GRBA_N_002") %>%
  ggplot(aes(x = factor(visitNumber), y = count, fill = factor(scientificName))) +
  geom_col()  +
...
```{r}
relativeDensityGraph <- lavo$data$LiveTrees %>%
  getRelativeDensity(grouping = sampleFrame) %>%
  ggplot(aes(x = factor(visitNumber), y = relativeDensity, fill = factor(scientificName))) +
  geom_col()  +
  facet_wrap(~sampleFrame) + 
...
```{r}
# Original version
relativeFrequency <- lavo$data$LiveTrees %>%
  getFrequency(grouping = sampleFrame) %>%
  ggplot(aes(x = factor(visitNumber), y = relativeFrequency, fill = factor(scientificName))) +
  geom_col() +
  facet_wrap(~sampleFrame) +
...
```{r}
relDomiance <- lavo$data$LiveTrees %>%
  getDominance(grouping = sampleFrame) %>%
  ggplot(aes(x = factor(visitNumber), y = relativeDominance, fill = factor(scientificName))) +
  geom_col() +
  facet_wrap(~sampleFrame) + 
...
```{r}
importanceVal <- lavo$data$LiveTrees %>%
  getImportanceValue(grouping = sampleFrame) %>%
  ggplot(aes(x = factor(visitNumber), y = importanceValue, fill = factor(scientificName))) +
  geom_col() +
  facet_wrap(~sampleFrame) + 
...
```{r}
countDeadGraph <- lavo$data$DeadTrees %>%
  getTreeCount(grouping = sampleFrame) %>%
  #filter(grepl("CRLA", locationID)) %>%
  #filter(locationID == "CRLA_Alpine_CRWB01") %>%
  group_by(scientificName, visitNumber, sampleFrame) %>%
...
# #/ha
# Density of every plot
densityGraph <- lavo$data$DeadTrees %>%
  #mutate(DBHGroup = cut(x = treeDBH_cm, breaks = 5*(0:(max(treeDBH_cm)/5)))) %>%
  group_by(scientificName, locationID, visitNumber) %>%
  summarise(density = n()/0.25) %>%
  group_by(scientificName) %>%
...
```{r}
densityPlotGraph <- lavo$data$DeadTrees %>%
  group_by(visitNumber, sampleFrame) %>%
  mutate(numberOfPlots = n_distinct(locationID)) %>%
  #mutate(DBHGroup = cut(x = treeDBH_cm, breaks = 5*(0:(max(treeDBH_cm)/5)))) %>%
  group_by(scientificName, visitNumber, sampleFrame) %>%
...
```{r}
countRecentlyDeadGraph <- lavo$data$DeadTrees %>%
  filter(vitality == "Recently Dead") %>%
  getTreeCount() %>%
  #filter(locationID == "GRBA_N_002") %>%
  ggplot(aes(x = factor(visitNumber), y = count, fill = factor(scientificName))) +
...
```{r}
densityRecentlyDead <- lavo$data$DeadTrees %>%
  filter(vitality == "Recently Dead") %>%
  group_by(scientificName, locationID, visitNumber) %>%
  summarise(density = n()/0.25) %>%
  ggplot(aes(x = factor(visitNumber), y = density, fill = factor(scientificName))) +
...
mortalityCause <- lavo$data$DeadTrees %>%
  filter(vitality == "Recently Dead") %>%
  group_by(locationID, visitNumber, causeOfDeath) %>%
  summarise(n = n()) %>%
  ggplot(aes(x = factor(visitNumber), y = n, fill = factor(causeOfDeath))) +
...
```{r}
# Data wrangling to calculate trees with mistletoe
mistletoePercentData <- lavo$data$Tree %>%
  filter(!is.na(mistletoe) & mistletoe != "ND") %>%
  group_by(scientificName, visitNumber) %>%
  summarise(percent = mean(mistletoe == 'Y')*100,
            mistletoeNum = sum(mistletoe == "Y"),
...
# plotly graph of mistletoe percent
if(nrow(mistletoePercentData) == 0) {
  print("No mistletoe infestation")
} else{
  plot_ly(data = mistletoePercentData,
          type = "bar",
          x = ~scientificName,
...
mistletoeDensity <- lavo$data$Tree %>%
  group_by(visitNumber) %>%
  # Find the total number of plots for the visit number
  mutate(totalPlots = n_distinct(locationID)) %>%
  filter(!is.na(mistletoe) & mistletoe != "ND") %>%
...
if(nrow(mistletoeDensity) == 0) {
  print("No mistletoe infestation")
} else{  
  mistletoeDensityGraph <- mistletoeDensity %>%
    ggplot(aes(x = factor(scientificName), y = avgDensity, fill = factor(visitNumber))) +
    geom_col(position = "dodge") + 
    theme_minimal() + 
    labs(title = "Mistletoe Density",
  ...
  ggplotly(mistletoeDensityGraph)
}
```
...
```{r}
femaleConePercent <- lavo$data$Tree %>%
  filter(!is.na(femaleCones)) %>%
  group_by(locationID, scientificName, year) %>%
  summarise(percent = mean(femaleCones == "Y")*100,
            yes = sum(femaleCones == "Y"),
...
femaleConeDensity <- lavo$data$Tree %>%
  filter(!is.na(femaleCones)) %>%
  group_by(scientificName, locationID, year) %>%
  summarise(density = sum(femaleCones == "Y")/0.25,
            n = n()) %>%
...
```{r}
# Total seedling counts of each species
seedlingCount <- lavo$data$Seedling %>%
  filter(heightClass != "ND") %>%
  getTreeCount(grouping = heightClass) %>%
  # group_by(scientificName) %>%
  # mutate(n = n()) %>%
...
# number/ha
#species, plot, height class
seedlingDensity <- lavo$data$Seedling %>%
  group_by(visitNumber) %>%
  mutate(totalPlots = n_distinct(locationID)) %>%
  filter(scientificName != "No seedlings" & scientificName != "Populus tremuloides") %>%
  filter(!is.na(heightClass)) %>%
...
seedlingFrequency <- lavo$data$Seedling %>%
  getFrequency(transectArea = 9, plotArea = 81) %>%
  group_by(visitNumber) %>%
  mutate(totalPlots = n_distinct(locationID)) %>%
  filter(scientificName != "No seedlings" & scientificName != "Populus tremuloides") %>%
...
seedlingRelDensity <- lavo$data$Seedling %>%
  getRelativeDensity(plotArea = 81) %>% 
  filter(scientificName != "No seedlings" & scientificName != "Populus tremuloides") %>%
  ggplot(aes(x = factor(visitNumber), y = relativeDensity, fill = scientificName)) +
  geom_col() +
...
```{r}
seedlingRelFrequency <- lavo$data$Seedling %>%
  getFrequency(transectArea = 9, plotArea = 81) %>% 
  filter(scientificName != "No seedlings" & scientificName != "Populus tremuloides") %>%
  ggplot(aes(x = factor(visitNumber), y = relativeFrequency, fill = scientificName)) +
  geom_col() +
