...
seedlingSpecs <- pine$data$Seedling %>%
  ...
  geom_bar(aes( fill = factor(scientificName))) +
  theme_minimal() + 
...
seedlingHeight <- pine$data$Seedling %>%
  dplyr::select(locationID, eventDate, scientificName, heightClass, eventID) %>%
...
seedingSiteGraph <- pine$data$Seedling %>%
  ...
  summarise(count = n()) %>%
  ggplot(aes(x = factor(locationID), y=count, fill = scientificName)) +
  geom_col() + 
  theme_minimal() + 
...
tree_pct <- pine$data$Tree %>%
  dplyr::select(locationID, eventID, scientificName) %>%
  full_join(pine$data$Site, by = "locationID") %>%
  dplyr::select(locationID, eventID, scientificName, primaryCornerYCoord) %>%
  group_by(primaryCornerYCoord, scientificName) %>%
...
tree_pct_plot <- tree_pct %>%
  ...
  geom_bar(aes(y = percent, fill = factor(scientificName)), stat = "identity", postion = "stack") + 
  theme_minimal() + 
...
treeElevation2Graph <- pine$data$Tree %>%
  ...
  full_join(sitesData, by = "locationID") %>%
  ggplot(aes(x = factor(sampleFrame), fill = as.factor(scientificName))) +
  geom_bar(stat = "count") + 
  theme_minimal() + 
...
heightBySpeciesGraph <- pine$data$Tree %>%
  ...
  ggplot(aes(x = scientificName, y = treeHeight_m, fill = scientificName)) +
  geom_boxplot() + 
  theme_minimal() + 
...
speciesDeadGraph <- pine$data$Tree %>%
  filter(vitality == "Recently Dead") %>%
  ggplot(aes(x = factor(year(as_date(eventDate))), fill = scientificName)) +
  geom_bar() + 
  # TODO: delete empty columns
  facet_wrap(~panel, scales="free_x") + 
  theme_minimal() + 
...
# remove duplicates from graph (for example most/all trees are counted twice)
# coarser bins for DBH in individual site DASHBOARD
```
