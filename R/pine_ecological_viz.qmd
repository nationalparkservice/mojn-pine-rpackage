...
frequencyData <- pine$data$LiveTrees %>%
  ...
  summarise(avgFrequency = sum(frequency)/n()) %>%
...
dominance <- pine$data$LiveTrees %>%
  ...
  filter(n > 4) %>%
  ggplot(aes(x = factor(scientificName), y = dominance, fill = factor(visitNumber))) +
...
```{r}
  frequency <- pine$data$LiveTrees %>%
    ...
    dplyr::group_by(locationID, scientificName, visitNumber, numberOfPlots) %>%
...
pine$data$LiveTrees %>%
  ...
  # Remove species with not enough data points to graph
  filter(n > 4) %>%
  ggplot(aes(x = factor(scientificName), y = treeHeight_m, fill = factor(visitNumber))) +
  facet_wrap(~scientificName, scales="free", drop= TRUE) +
  geom_violin() + 
  stat_summary(fun = median, geom = "point", position = position_dodge(0.9)) +
  stat_summary(fun = mean, geom = "point", position = position_dodge(0.9), color = "red") +
