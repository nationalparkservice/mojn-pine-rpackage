...
library(tidyverse)
library(plotly)
pine <- fiveneedlepine::loadPine("../FNP_MOJN_Primary.accdb")
...
output$treeStatusPlot <- renderPlotly({
  ...
  filter(locationID == input$siteCode) %>%
  ggplot(aes(x = factor(lubridate::year(eventDate)), fill = vitality)) +
  geom_bar() + 
  theme_minimal() + 
...
output$coneCountPlot <- renderPlotly({
  coneCountGraph <- pine$data$Tree %>%
    ...
    ggplot(aes(x = factor(year),y = coneCountTotal, fill = scientificName)) +
    geom_col() + 
    theme_minimal() + 
...
output$treeHeightPlot <- renderPlotly({
  treeHeight <- pine$data$Tree %>%
    ...
    mutate(year = factor(lubridate::year(eventDate))) %>%
...
output$seedlingCountPlot <- renderPlotly({
  seedlingCount <- pine$data$Seedling %>%
    ...
    group_by(year, scientificName) %>%
    summarise(count = n()) %>%
    ggplot(aes(x = year, y = count, fill = scientificName)) +
    geom_col() +
    theme_minimal() +
...
output$aliveSpeciesPlot <- renderPlotly({
  treeSpeciesPercent <- pine$data$Tree %>%
    ...
    ggplot(aes(x = factor(year), y = percent, fill = scientificName)) +
    geom_col() +
    theme_minimal() +
